...
from mutab.otsl import html_to_otsl

EASY = "simple"
HARD = "complex"


class TabularTransform(ABC, BaseTransform):
...
@TRANSFORMS.register_module()
class FillBbox(TabularTransform):
    def __init__(self, cell: str, **kwargs):
        super().__init__()
    ...
    def progress(self, html, cell, **kwargs):
        ...
        for item in cell:
            list_cell.append(item.get("cell"))
...
@TRANSFORMS.register_module()
class FlipBbox(TabularTransform):
...
@TRANSFORMS.register_module()
class FormBbox(TabularTransform):
